...
export class SessionRuntime {
  ...
  private baseUsage = { inputTokens: 0, outputTokens: 0, cacheReadTokens: 0 }
  /** Cumulative token totals reported by the current process, as of last turn. */
  private processUsage = { inputTokens: 0, outputTokens: 0, cacheReadTokens: 0 }
  ...
  constructor(
    ...
    this.baseUsage = {
      inputTokens: meta.stats.inputTokens,
      outputTokens: meta.stats.outputTokens,
      cacheReadTokens: meta.stats.cacheReadTokens
    }
  }
    ...
    for (const event of events) {
      switch (event.t) {
        ...
        case 'turn-complete': {
          ...
          // Prefer the cumulative modelUsage accounting; the per-turn usage
          // field only covers the main loop and is zeroed on some SDK paths.
          let turnUsage = event.usage
          if (event.cumulativeUsage) {
            turnUsage = {
              inputTokens: Math.max(0, event.cumulativeUsage.inputTokens - this.processUsage.inputTokens),
              outputTokens: Math.max(0, event.cumulativeUsage.outputTokens - this.processUsage.outputTokens),
              cacheReadTokens: Math.max(0, event.cumulativeUsage.cacheReadTokens - this.processUsage.cacheReadTokens)
            }
            this.processUsage = { ...event.cumulativeUsage }
            event.usage = turnUsage
          } else {
            this.processUsage = {
              inputTokens: this.processUsage.inputTokens + turnUsage.inputTokens,
              outputTokens: this.processUsage.outputTokens + turnUsage.outputTokens,
              cacheReadTokens: this.processUsage.cacheReadTokens + turnUsage.cacheReadTokens
            }
          }

          this.meta.stats = {
            totalCostUsd: this.baseCostUsd + this.processCost,
            inputTokens: this.baseUsage.inputTokens + this.processUsage.inputTokens,
