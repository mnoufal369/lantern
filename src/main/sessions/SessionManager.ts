...
export class SessionManager {
  ...
  private maybeNotify(meta: SessionMeta, status: SessionStatus): void {
    ...
    const title = meta.title || 'Lantern session'
  ...
  /** `lantern <dir>` CLI entry: focus the existing tab for a folder or open a new one. */
  ...
  /** Past Claude Code sessions from the shared store, minus ones Lantern already owns. */
  ...
  /** Adopts a terminal session: imports its transcript and makes it resumable in Lantern. */
