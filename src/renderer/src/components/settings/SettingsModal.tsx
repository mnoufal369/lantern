...
export default function SettingsModal({ onClose }: { onClose: () => void }): React.JSX.Element {
  ...
  const save = async (): Promise<void> => {
    setSaving(true)
    const patch: { apiKey?: string; maxConcurrentSessions: number; customInstructions: string; githubOrg: string } = {
  ...
  return (
    <Modal title="Settings" onClose={onClose}>
      <div className="space-y-4">
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-zinc-400">GitHub organisation</label>
          <input
            value={githubOrg}
          ...
          />
          <p className="mt-1 text-[11px] text-zinc-600">
            New Session pre-fills github.com/&lt;org&gt;/ and suggests its repositories as you type (uses the gh CLI
            if installed).
          </p>
        </div>
