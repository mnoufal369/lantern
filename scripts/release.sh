...
ASSETS=()
for FILE in \
  "release/Lantern-$VERSION-mac-arm64.zip" \
  "release/Lantern-$VERSION-arm64.dmg" \
  "release/Lantern-Setup-$VERSION-x64.exe"
do
  [ -f "$FILE" ] && ASSETS+=("$FILE")
done
...
if gh release view "$TAG" >/dev/null 2>&1; then
  ...
  gh release create "$TAG" "${ASSETS[@]}" --title "Lantern $VERSION" --notes "See CHANGELOG.md" --latest
fi
